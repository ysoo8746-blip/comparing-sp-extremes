...
####################################################
### Extract FDRs and Power 
####################################################

MyMulti_outputNeiN = function(f.name=NULL,N1=NULL, Param,Method=m, name,Nei=1){
  
  ### Existing method 
  if (!is.null(f.name)){
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/JCGS/Rcode_Submit/Result/',Param, "_", f.name)
  } else {
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/JCGS/Rcode_Submit/Result/',Param,"_Rt_Size", N1, "Diff_P33_R3S0.2B9_", name)
  }
  setwd(dir.name)
  ...
  if (!is.null(f.name)){
    ...
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/JCGS/Rcode_Submit/Result/',Param,"_Rt_Size", N1, "Diff_P33_R3S0.2B", name)
  }
  setwd(dir.name)
  ...
  FDR_Bi4 = apply(FDR_Bi4, 2, function(x) replace(x, is.na(x),0)); FDR_Bi4 = apply(FDR_Bi4, 2, function(x) replace(x, which(x >0.8),0))
  FDR_Bi4 = cbind(FDR_Bi[,-4], FDR_Bi4[,4])
  
  FDR_Bi12 = apply(FDR_Bi12, 2, function(x) replace(x, is.na(x),0)); FDR_Bi12 = apply(FDR_Bi12, 2, function(x) replace(x, which(x >0.8),0))
  FDR_Bi12 = cbind(FDR_Bi[,-4], FDR_Bi12[,4])
  
  FDR_BiP10_2 = apply(FDR_BiP10_2, 2, function(x) replace(x, is.na(x),0)); FDR_BiP10_2 = apply(FDR_BiP10_2, 2, function(x) replace(x, which(x >0.8),0)) 
  FDR_BiP10_2 = cbind(FDR_BiP10[,-4], FDR_BiP10_2[,4])
  FDR_BiP10_4 = apply(FDR_BiP10_4, 2, function(x) replace(x, is.na(x),0));FDR_BiP10_4 = apply(FDR_BiP10_4, 2, function(x) replace(x, which(x >0.8),0))
  FDR_BiP10_4 = cbind(FDR_BiP10[,-4], FDR_BiP10_4[,4]);
  FDR_BiP10_12 = apply(FDR_BiP10_12, 2, function(x) replace(x, is.na(x),0));  FDR_BiP10_12 = apply(FDR_BiP10_12, 2, function(x) replace(x, which(x>0.8),0))
  FDR_BiP10_12 = cbind(FDR_BiP10[,-4], FDR_BiP10_12[,4]);

  FDR_BiP50_2 = apply(FDR_BiP50_2, 2, function(x) replace(x, is.na(x),0)); FDR_BiP50_2 = apply(FDR_BiP50_2, 2, function(x) replace(x, which(x >0.8),0))
  FDR_BiP50_2 = cbind(FDR_BiP50[,-4], FDR_BiP50_2[,4])
  FDR_BiP50_4 = apply(FDR_BiP50_4, 2, function(x) replace(x, is.na(x),0)); FDR_BiP50_4 = apply(FDR_BiP50_4, 2, function(x) replace(x, which(x >0.8),0))
  FDR_BiP50_4 = cbind(FDR_BiP50[,-4], FDR_BiP50_4[,4]);
  FDR_BiP50_12 = apply(FDR_BiP50_12, 2, function(x) replace(x, is.na(x),0));FDR_BiP50_12 = apply(FDR_BiP50_12, 2, function(x) replace(x, which(x >0.8),0)) 
...
MyMulti_outputNeiLoc = function(f.name0, Param,Method=m, Nei=1){
  
  ### Existing method 
  if (!is.null(f.name0)){
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/JCGS/Rcode_Submit/Result/',Param,"_Rt_", f.name0)
  } else {
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/JCGS/Rcode_Submit/Result/',Param,"_Rt_C", N1, "K", N2)
  }
  setwd(dir.name)
  ...
  ### Aggregating N neighbors
  if (!is.null(f.name0)){
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/JCGS/Revision/data/Simulation/',Param,"_Rt_", f.name0)
  } else {
    dir.name= paste0('/Users/syun/Baruch College Dropbox/Soo In Yun/Research/Comparing Extremes/newdata/NAM/Simulation_Precip_Temp/',Param,"_Rt_C", N1, "K", N2)
  }
  setwd(dir.name)
...
### Estimation Plot
######################################################
reshape_loc_diff <- function(diff_vec, signal_label, delta_vec) {
  tibble(
    location_id = 1:length(diff_vec),
    loc_diff = diff_vec,
    delta = delta_vec,
...
make_df <- function(vec10, vec50, strength_label, delta_vec) {
  stopifnot(length(vec10) == length(vec50), length(vec10) == length(delta_vec))
  n <- length(vec10)
  tibble(
    loc_id   = rep(seq_len(n), 2),
    diff     = c(vec10, vec50),
    horizon  = factor(rep(c("10-year", "50-year"), each = n), levels = c("10-year", "50-year")),
    strength = factor(strength_label, levels = c("Weak", "Moderate", "Strong")),
    delta    = factor(rep(delta_vec, 2), levels = c(0, 1), labels = c("Null", "Alternative"))
  )
}
